...
// NETCDF FILE STUFF
    ...
    // READ VARIABLES FROM DE NETCDF FILE
    ...
    glbParam.nEvents = size_dim[0] ; glbParam.nEventsAVG = size_dim[0] ;
    ...
    // double  dataFile[glbParam.nEventsAVG][glbParam.nCh][glbParam.nBins];
...
// READ GLOBAL PARAMETERS FROM NETCDF FILE
    ...
    nc_get_att_double(	(int)ncid, (int)NC_GLOBAL, (const char*)"Range_Resolution", (double*)&glbParam.dr) ;
    glbParam.r = (double*) new double[glbParam.nBins] ;
    for( int i=0 ; i <glbParam.nBins ; i++ )
        glbParam.r[i] = i*glbParam.dr ;

    nc_get_att_double(	(int)ncid, (int)NC_GLOBAL, (const char*)"Altitude_meter_asl", (double*)&glbParam.siteASL) ;

    int indxWL_PDL1 ;
    ReadAnalisysParameter( (char*)glbParam.FILE_PARAMETERS, (const char*)"indxWL_PDL1", (const char*)"int", (int*)&indxWL_PDL1 ) ;
    assert( indxWL_PDL1 <= (glbParam.nLambda -1 ) ) ;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for ( int e=0 ; e <glbParam.nEventsAVG ; e++ )
    {
        ...
        for ( int c=0 ; c <glbParam.nCh ; c++ )
        {
            ...
            for(int b =0; b <(glbParam.nBins - glbParam.indxOffset); b++)
                pr_corr[e][c][b] = (double)dataFile[e][c][b+glbParam.indxOffset-1] ; // BIN OFFSET CORRECTION;
            for ( int b=(glbParam.nBins -glbParam.indxOffset) ; b <glbParam.nBins ; b++ )
                pr_corr[e][c][b] = (double)dataFile[e][c][ glbParam.nBins- glbParam.indxOffset-1 ] ; // BIN OFFSET CORRECTION;
        }
    }

    double  **pr2 = (double**) new double*[glbParam.nEventsAVG];
    for ( int e=0 ; e <glbParam.nEventsAVG ; e++ )
        pr2[e] = (double*) new double[glbParam.nBins] ;
    ...
    for ( int t=0 ; t <glbParam.nEvents ; t++ )
    {
        ...
        for ( int i=0 ; i <(glbParam.nBins -glbParam.indxOffset) ; i++ )
            evSig.pr[i] = (double) dataFile[t][indxWL_PDL1][i+glbParam.indxOffset-1] ; // BIN OFFSET CORRECTION
        for ( int i=(glbParam.nBins -glbParam.indxOffset) ; i <glbParam.nBins ; i++ )
            evSig.pr[i] = (double) dataFile[t][indxWL_PDL1][ glbParam.nBins- glbParam.indxOffset-1 ] ;
        ...
        oDL1.MakeRangeCorrected    ( (strcLidarSignal*)&evSig, (strcGlobalParameters*)&glbParam, (strcMolecularData*)&oMolData->dataMol[indxWL_PDL1] ) ;
        for( int i=0 ; i <glbParam.nBins ; i++ )
            pr2[t][i] = (double)evSig.pr2[i] ;
    ...
    oNCL.Save_LALINET_NCDF_PDL1( (string*)&Path_File_In, (string*)&Path_File_Out, (strcGlobalParameters*)&glbParam, (int**)clouds_ON_mtx, (double***)pr_corr, (double**)pr2 ) ;
