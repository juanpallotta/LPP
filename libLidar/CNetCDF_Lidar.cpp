...
void CNetCDF_Lidar::Read_GlbParameters( int ncid, strcGlobalParameters *glbParam )
{
    ...
    size_t size_dim[3] ;
        for ( int d=0 ; d <num_dim_var ; d++ ) size_dim[d] =(size_t)1 ;
    for( int d=0 ; d <num_dim_var ; d++ )
    {
        if ( ( retval = nc_inq_dimlen( (int)ncid, (int)id_dim[d], &size_dim[d] ) ) )
            ERR(retval);
    ...
    int num_Ch = ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"indxOffset" , (const char*)"int"   , (int*)    glbParam->indxOffset ) ;
    ...
    glbParam->nBins_Ch_eff = (int*) new int[ glbParam->nCh ] ;
...
// printf("\n PDL1: glbParam->nEvents= %d \t glbParam->numEventsToAvg= %d \t glbParam->nEventsAVG= %d \n", glbParam->nEvents, glbParam->numEventsToAvg, glbParam->nEventsAVG) ;
    ...
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"Temperature_at_Lidar_Station_K", (const char*)"double", (int*)&glbParam->temp_K_agl[0]  ) ;
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"Pressure_at_Lidar_Station_Pa"  , (const char*)"double", (int*)&glbParam->pres_Pa_agl[0] ) ;
    ...
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"nBinsBkg"        , (const char*)"int"   , (int*)   &glbParam->nBinsBkg        ) ;
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"range_Bkg_Start" , (const char*)"double", (double*)&glbParam->range_Bkg_Start ) ;
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"range_Bkg_Stop"  , (const char*)"double", (double*)&glbParam->range_Bkg_Stop  ) ;
    
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"rInitSig"   , (const char*)"double", (double*)&glbParam->rInitSig   ) ;

    {
        int *indxWL_PDL1_nCh = (int*) malloc( glbParam->nCh * sizeof(int) ) ;    
        int nCh1 = ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"indxWL_PDL1", (const char*)"int", indxWL_PDL1_nCh ) ;
        glbParam->indxWL_PDL1 = (int*) malloc( nCh1 * sizeof(int) ) ;
        glbParam->nCh_to_process_L1 = nCh1 ;
        for (int i = 0; i < nCh1; i++) glbParam->indxWL_PDL1[i] = indxWL_PDL1_nCh[i];
        free(indxWL_PDL1_nCh);

        int *indxWL_PDL2_nCh = (int*) malloc( glbParam->nCh * sizeof(int) ) ;    
        int nCh2 = ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"indxWL_PDL2", (const char*)"int", indxWL_PDL2_nCh ) ;
        glbParam->indxWL_PDL2 = (int*) malloc( nCh2 * sizeof(int) ) ;
        glbParam->nCh_to_process_L2 = nCh2 ;
        for (int i = 0; i < nCh2; i++) glbParam->indxWL_PDL2[i] = indxWL_PDL2_nCh[i];
        free(indxWL_PDL2_nCh);
    }
    ...
    for ( int e =0 ; e <glbParam->nEventsAVG ; e++ )
    {
        ...
        for (int c =0 ; c <glbParam->nCh ; c++)
        {
            glbParam->rEndSig_ev_ch   [e][c] = (double) -1000.0 ; 
            glbParam->indxEndSig_ev_ch[e][c] = (int)    -1000   ; 
        }
    }
...
void CNetCDF_Lidar::Read_L0_data( int ncid, strcGlobalParameters *glbParam, CDataLevel_2 *oDL2 )
{
    ...
    allocate_memory_matrix( (void*)&oDL2->data_File_L0, 3, dim_sizes, "double" ) ;
...
void CNetCDF_Lidar::Read_L1_into_L2( int ncid_L1_Data, strcGlobalParameters *glbParam, CDataLevel_2 *oDL2 )
{
    ...
    // Layer_Mask VARIABLE IS ALWAY PRESENT IN THE L1 DATA GROUP.
    int id_var_lm ;
    if ( ( retval = nc_inq_varid( (int)ncid_L1_Data, (const char*)"Layer_Mask", (int*)&id_var_lm ) ) )
        ERR(retval) ;
    ...
    {
        start_lm[0] =t ;
        if ( (retval = nc_get_vara_int((int)ncid_L1_Data, (int)id_var_lm, start_lm, count_lm, (int*)&oDL2->oLOp->cloudProfiles[t].clouds_ON[0] ) ) )
            ERR(retval) ;
    }
}
...
void CNetCDF_Lidar::Save_SCC_NCDF_Format( string Path_File_Out, strcGlobalParameters *glbParam, double ***dataToSave, long *Raw_Data_Start_Time, string *Raw_Data_Start_Time_str, long *Raw_Data_Stop_Time, string *Raw_Data_Stop_Time_str )
{
    ...
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"Molecular_Calc"              , (const char*)"int"   , (int*)   &Molecular_Calc ) ;
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"Pressure_at_Lidar_Station_Pa"   , (const char*)"double", (double*)&Pressure_at_Lidar_Station_Pa    ) ;
    ReadAnalysisParameter( (char*)glbParam->FILE_PARAMETERS, (const char*)"Temperature_at_Lidar_Station_K", (const char*)"double", (double*)&Temperature_at_Lidar_Station_K ) ;
...
void CNetCDF_Lidar::Save_LALINET_NCDF_PDL1( char *Path_File_Out, strcGlobalParameters *glbParam, CDataLevel_1 *oDL1, double ***pr_corr, int *Start_Time_AVG, int *Stop_Time_AVG, CMolecularData *oMolData )
{
    ...
    int nc_id_group_L1_MolData ;
    if ( (retval = nc_def_grp ( (int)nc_id_group_L1, (const char*)"Molecular_Data", (int*)&nc_id_group_L1_MolData ) ) )
        ERR(retval) ;
    ...
    DefineVariable( (int)nc_id_group_L1, (char*)"Layer_Mask", (const char*)"int", (int)2, (int*)&dim_ids_CM[0], (int*)&var_ids[12] ) ;
    ...
    for( int e=0 ; e <glbParam->nEventsAVG  ; e++ )
    {
        ...
        if ( (retval = nc_put_vara_double( (int)nc_id_group_L1, (int)var_ids[18], start_ES, count_ES, (double*)&oMolData->dataMol.last_Range_Bkg_Low[e][0] ) ) )
            ERR(retval);
        if ( (retval = nc_put_vara_double( (int)nc_id_group_L1, (int)var_ids[19], start_ES, count_ES, (double*)&oMolData->dataMol.last_Range_Bkg_High[e][0] ) ) )
            ERR(retval);
    }
...
// id_dims_aer[2]: nBins
    ...
    if ( glbParam->iLambda[glbParam->indxWL_PDL2[0]] == 355 )
    {
    ...
    else if ( glbParam->iLambda[glbParam->indxWL_PDL2[0]] == 351 )
    {
    ...
    else if ( glbParam->iLambda[glbParam->indxWL_PDL2[0]] == 532 )
    {
    ...
    else if ( glbParam->iLambda[glbParam->indxWL_PDL2[0]] == 1064 )
    {
    ...
    int     indxWL_PDL2 ;
    double  R_ref       ;
    ReadAnalysisParameter( (const char*)glbParam->FILE_PARAMETERS, (const char*)"indxWL_PDL2", (const char*)"int"    , (int*)&indxWL_PDL2 ) ;
    ReadAnalysisParameter( (const char*)glbParam->FILE_PARAMETERS, (const char*)"R_ref"      , (const char*)"double" , (double*)&R_ref    ) ;
