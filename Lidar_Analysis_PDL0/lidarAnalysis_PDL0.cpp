...
int main( int argc, char *argv[] )
{
    ...
    { // A *FOLDER* WAS PASSED AS AN ARGUMENT --> analyze all the files within the time bin set in analysisParameter.dat
        ...
        nFilesInTime = (int)getInputFilesInTimeRange((char*)Path_In.c_str(), (char**)inputFilesInTime, (strcGlobalParameters*)&glbParam ) ;
    }
    else
    {
...
// MAIN LOOP ACROSS THE CLUSTER FILES ////////////////////////////////////////////////////////////////////////////
    for ( int fC=0 ; fC <glbParam.nEventsAVG ; fC++ )
    {
        ...
        for ( int f=0 ; f <glbParam.nEvents ; f++ ) // glbParam.nEvents = NUMBER OF FILES TO AVERAGE.
        {
            ...
            printf( "\t %s \t (sec: %ld) \n", inputFilesInTime[fC*glbParam.nEvents +f], avgTime_num ) ;
            ...
            glbParam.aAzimuthAVG[fC] = glbParam.aAzimuthAVG[fC] /glbParam.nEvents ;
            glbParam.aZenithAVG[fC]  = glbParam.aZenithAVG[fC]  /glbParam.nEvents ;
            glbParam.temp_CelsiusAVG[fC] = glbParam.temp_CelsiusAVG[fC]  /glbParam.nEvents ;
            glbParam.pres_hPaAVG[fC]     = glbParam.pres_hPaAVG[fC]       /glbParam.nEvents ;
                // FOLDER AND FILE NAME GENERATION.
                avgTime_num   = (time_t) round( avgTime_num /glbParam.nEvents ) ;
                // Raw_Data_AVG_Time[fC]   = (int)avgTime_num ;
                *tmFile_avg = *localtime( &avgTime_num ) ;
                tmFile_avg->tm_year = 1900 + tmFile_avg->tm_year ;

                mkdir  ( Path_Out.c_str(), 0777 ) ; // mkdir  ( PathMergedFiles, 0777 ) ;
        ...
        for ( int c=0 ; c <glbParam.nCh ; c++ )
        {
            for ( int b=0 ; b <glbParam.nBinsRaw ; b++ )
            {
                ...
                dataToSave[fC][c][b]        = (double) round(dataToSave[fC][c][b] /glbParam.nEvents) ;
            }
        }
        // for ( int f=0 ; f <glbParam.nEvents ; f++ )
...
// NETCDF FILE STUFF
    ...
    if ( strcmp(glbParam.outputDataFileFormat, "SCC_NETCDF") ==0 )
    {
        ...
        oNCL.Save_SCC_NCDF_Format( (string)Path_File_Out, (strcGlobalParameters*)&glbParam, (double***)dataToSave, (int*)Raw_Data_Start_Time, (string*)Raw_Data_Start_Time_str, (int*)Raw_Data_Stop_Time, (string*)Raw_Data_Stop_Time_str ) ;
    }
