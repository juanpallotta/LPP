...
2. Aerosol_Extinction_<X>nm variables from the L2_Data group (if it exists) as 1D plots (Data level 2 plots).
...
def plot_cloud_mask(dataset, output_dir=None):
    ...
    if not check_variable_exists(l1_group, 'Cloud_Mask'):
        print("Variable 'Cloud_Mask' does not exist in group 'L1_Data'.")
        return
    
    # Check if Start_Time_AVG_L1 variable exists
    if not check_variable_exists(l1_group, 'Start_Time_AVG_L1'):
        print("\033[91m" + "Warning: Variable 'Start_Time_AVG_L1' does not exist in group 'L1_Data'." + "\033[0m")
        return
    
    # Check if range variable exists
    if not check_variable_exists(l1_group, 'range'):
        print("\033[91m" + "Warning: Variable 'range' does not exist in group 'L1_Data'." + "\033[0m")
        return
    ...
    ax.set_title('Cloud Mask', fontsize=14, fontweight='bold')
    
    # Limit range to 20000 meters
    ax.set_ylim(0, 20000)
    
    # Add colorbar
    cbar = plt.colorbar(im, ax=ax, label='Cloud Mask')
    cbar.set_ticks([0, 1])
...
def plot_aerosol_extinction(dataset, time_index=0, output_dir=None):
    ...
    if not aerosol_vars:
        print("No Aerosol_Extinction_<X>nm variables found in group 'L2_Data'.")
        return
    ...
    if not check_variable_exists(l2_group, 'range'):
        print("\033[91m" + "Warning: Variable 'range' does not exist in group 'L2_Data'." + "\033[0m")
        return
    ...
    for var_name in aerosol_vars:
        ...
        # Check dimensions
        if len(var.shape) != 3:
            print("\033[91m" + f"Warning: Variable '{var_name}' does not have 3 dimensions. Skipping." + "\033[0m")
            continue
        ...
        if time_index < 0 or time_index >= time_dim:
            print("\033[91m" + f"Warning: Time index {time_index} is out of bounds [0, {time_dim-1}]. Using index 0." + "\033[0m")
            time_index = 0
...
def plot_range_corrected_lidar_signal(dataset, time_index=0, output_dir=None):
    ...
    if not check_variable_exists(l2_group, 'Range_Corrected_Lidar_Signal_L2'):
        print("\033[91m" + "Warning: Variable 'Range_Corrected_Lidar_Signal_L2' does not exist in group 'L2_Data'." + "\033[0m")
        return
    ...
    try:
        ...
        print("\033[91m" + "Warning: Attribute 'indxChannel_for_Fernald_inv' not found in L2_Data. Using 0." + "\033[0m")
        channel_index = 0
    ...
    # Validate indices
    if time_index < 0 or time_index >= time_dim:
        print("\033[91m" + f"Warning: Time index {time_index} is out of bounds [0, {time_dim-1}]. Using 0." + "\033[0m")
        time_index = 0
        
    if channel_index < 0 or channel_index >= chan_dim:
        print("\033[91m" + f"Warning: Channel index {channel_index} is out of bounds [0, {chan_dim-1}]. Using 0." + "\033[0m")
        channel_index = 0
...
def plot_range_corrected_lidar_signal_colormap(dataset, output_dir=None):
    ...
    # Check if L2_Data group exists
    if not check_group_exists(dataset, 'L2_Data'):
        print("Group 'L2_Data' does not exist in the NetCDF file for ColorMap.")
        return
