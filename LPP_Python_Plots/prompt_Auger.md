...
## Data level 2 plots
    -   Inside group "L2_Data" (check if this group exists):
        ...
        - Variables "L2_Data/Aerosol_Backscatter_<X>nm" and "L2_Data/Aerosol_Extinction_<X>nm" (time, LRs, range). Use the time slice corresponding to the second argument of the script. The LRs slice must be taken as the middle index of the dimension LRs of the matrix. The y-axis must be taken from the variable "/L2_Data/range", which is a vector and contain the range in meters. Write the units of each variable. Draw the grid lines of the plot. In the title, put the time in human-readable format and the LRs selected. Save the plot as a PNG file and name it as "Aerosol_Backscatter_532nm.png" and "Aerosol_Extinction_532nm.png". 

        -   Variable: "L2_Data/Aerosol_Backscatter_<X>nm". It is a 3D Matrix (time, channels, range), and should be plotted as a colormap. For the plotting, follows the instruction of the variable "Cloud_Mask" in the group "L1_Data". In the title, put the time in human-readable format and the channels selected. 
